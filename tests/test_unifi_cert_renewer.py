...
def test_live_exact_verification_precedes_finalisation_and_completion(
    ...
    def verify(**kwargs):
        ...
        assert kwargs["endpoint"] == LiveTLSEndpoint("127.0.0.1", "unifi.test", 8443)
        assert kwargs["trusted_ca_data"] == arguments["trusted_ca_data"]
        assert (
    ...
    monkeypatch.setattr(renewer, "verify_live_tls_certificate", verify)
...
def test_stage7_safe_failure_categories_are_distinct(
    ...
    monkeypatch.setattr(renewer, "verify_live_tls_certificate", fail)
